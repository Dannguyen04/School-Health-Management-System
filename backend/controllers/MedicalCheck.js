...
const createMedicalCheck = async (req, res) => {
    try {
        ...
        if (
            ...
            return res.status(400).json({
                success: false,
                error: "Thiếu thông tin bắt buộc cho báo cáo kiểm tra sức khỏe. Vui lòng nhập đầy đủ tất cả các trường khám thể lực và lâm sàng!",
            });
        }
        ...
        if (
            !["EXCELLENT", "GOOD", "AVERAGE", "WEAK"].includes(
                physicalClassification
            )
        )
            return res.status(400).json({
        ...
        const avgHearing = (
            (hearingLeftNormal + hearingRightNormal) /
            2
        ).toFixed(1);
