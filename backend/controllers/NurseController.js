...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    const uniqueApprovedMedications = new Set(
      approvedMedications.map((med) => `${med.name}|${med.dosage}|${med.unit}`)
    ).size;
  ...
  try {
    ...
    if (category) {
      medications = medications.filter(
        (med) =>
          (med.description || "").toLowerCase() === category.toLowerCase()
      );
    }
      ...
      if (previousDoses.length >= vaccine.maxDoseCount) {
        return res.status(400).json({
          ...
          details: {
            ...
            requestedDoseOrder: doseOrder,
            vaccineName: vaccine.name,
            completedDoseOrders: previousDoses.map((d) => d.doseOrder).sort(),
          },
        });
      }
    }
      ...
      if (medicationRequest.parent && medicationRequest.parent.user) {
        await prisma.notification.create({
          data: {
            ...
            message: `Yêu cầu thuốc ${medicationRequest.name} cho học sinh ${
...
export const getScheduledTreatments = async (req, res) => {
  try {
    ...
    const upcomingNotifications = scheduledTreatments
      .filter((treatment) => {
        if (!treatment.todaySchedules || treatment.todaySchedules.length === 0)
          return false;
