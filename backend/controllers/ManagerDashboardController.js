...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    campaigns.forEach((c) => {
      ...
      agreedConsents += c.consents.filter((x) => x.consent === true).length;
      declinedConsents += c.consents.filter((x) => x.consent === false).length;
    });

    // Số học sinh đã tiêm chủng (unique studentId trong vaccinations, status = COMPLETED)
    const vaccinatedStudents = await prisma.vaccinations.aggregate({
    ...
    vaccinations.forEach((v) => {
      for (const grade in gradeMap) {
        if (gradeMap[grade].includes(v.studentId)) {
          vaccinatedByGrade[grade] = (vaccinatedByGrade[grade] || 0) + 1;
        }
      }
    });
