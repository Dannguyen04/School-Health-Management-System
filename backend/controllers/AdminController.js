...
const addStudent = async (req, res) => {
  try {
    ...
    const validGenders = ["male", "female", "other", "nam", "nu", "Nam", "Nữ"];
...
const updateStudent = async (req, res) => {
  ...
  try {
    ...
    // Validate gender
    console.log("🔍 Backend received gender:", gender, "Type:", typeof gender);
    ...
    if (!validGenders.includes(gender.toLowerCase())) {
      console.log("❌ Invalid gender:", gender, "Valid options:", validGenders);
      return res.status(422).json({
        success: false,
        error: `Giới tính phải là: ${validGenders.join(", ")}`,
      });
