  ...
  try {
    ...
    if (!name || !vaccineId || !scheduledDate || !deadline) {
      return res.status(400).json({
        success: false,
        error:
          "Thiếu thông tin bắt buộc: name, vaccineId, scheduledDate, deadline",
      });
    }
      ...
      await prisma.notification.create({
        data: {
          ...
          message: `Bạn đã tạo thành công chiến dịch tiêm chủng "${
            campaign.name
          }" với vaccine ${vaccine.name}. Chiến dịch sẽ diễn ra từ ${new Date(
            scheduledDate
          ).toLocaleDateString("vi-VN")} đến ${new Date(
            deadline
          ).toLocaleDateString("vi-VN")} cho các khối: ${targetGrades.join(
            ", "
          )}.`,
...
const getAllVaccinationCampaigns = async (req, res) => {
  try {
    ...
    const campaignsWithAcademicYear = campaigns.map((campaign) => ({
      ...campaign,
      calculatedAcademicYear: getAcademicYearFromDate(campaign.scheduledDate),
    }));
  ...
  try {
    ...
    if (parentUserIds.length === 0) {
      return res.status(400).json({
        success: false,
        error:
          "Không tìm thấy phụ huynh nào cho các học sinh đủ điều kiện trong các khối đã chọn",
      });
    }
