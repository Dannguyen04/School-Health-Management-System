...
const getOverallSummaryReport = async (filter) => {
  ...
  try {
    ...
    const summary = {
      ...
      vision: {
        ...visionStats,
        percentages: calculatePercentages(visionStats, visionStats.total),
      },
    };
  ...
  try {
    ...
    return {
      ...
      summary: {
        ...
        followUpRate:
          attentionCount > 0
            ? Math.round((followUpCount / attentionCount) * 10000) / 100
            : 0,
      },
    };
  } catch (error) {
...
const getStudentsNeedingAttention = async (req, res) => {
  try {
    ...
    return handleError(error, res, "Failed to get students needing attention");
  }
};
