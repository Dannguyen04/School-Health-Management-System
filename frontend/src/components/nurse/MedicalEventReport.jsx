...
const MedicalEventReport = () => {
  ...
  useEffect(() => {
    ...
    if (searchText?.trim()) {
      ...
      filtered = filtered.filter(
        (event) =>
          event.studentName.toLowerCase().includes(searchTerm) ||
  ...
  useEffect(() => {
    ...
    if (studentSearchText?.trim()) {
      const searchTerm = studentSearchText.trim().toLowerCase();
      filtered = filtered.filter(
        (student) =>
          ...
          {hasActiveFilters() && (
            <div className="border-t pt-4">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span>sự kiện</span>
                {filteredEvents.length !== medicalEvents.length && (
                  <>
                    <span>trong tổng số</span>
                    <span className="font-medium">{medicalEvents.length}</span>
                    <span>sự kiện</span>
                  </>
                )}
              </div>
      ...
      >
        <Form form={form} layout="vertical">
                ...
                >
                  <div className="flex gap-2">
                    <Input
                      ...
                      style={{ flex: 1 }}
                    />
                    <Button type="primary" onClick={openStudentModal}>
                      Chọn học sinh
                    </Button>
                  </div>
                </Form.Item>
          ...
          <div className="mb-6">
            ...
            <Form.Item name="treatment" label="Điều trị đã thực hiện">
              <TextArea
                rows={3}
            ...
            <Row gutter={16}>
              ...
              <Col span={12}>
                <Form.Item name="outcome" label="Kết quả cuối cùng">
        ...
        <div className="space-y-4">
          ...
          <div className="text-sm text-gray-600 text-center">
            ...
            {studentSearchText || studentFilterGrade ? (
              <span> (trong tổng số {students.length} học sinh)</span>
            ) : null}
          </div>
        </div>
      </Modal>
