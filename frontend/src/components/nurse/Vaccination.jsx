...
const Vaccination = () => {
    ...
    const validateReaction = (_, value) => {
        ...
        if (sideEffects && sideEffects.trim() !== "" && value === "NONE") {
            return Promise.reject(
                new Error(
                    'Nếu có tác dụng phụ thì không thể "Không có phản ứng"'
                )
            );
        }
    ...
    const validateSideEffects = (_, value) => {
        ...
        if (value && value.trim() !== "" && reaction === "NONE") {
            return Promise.reject(
                new Error(
                    'Nếu có tác dụng phụ thì không thể chọn "Không có phản ứng"'
                )
            );
        }
    ...
    const validateDoseOrder = (_, value) => {
        ...
        // Kiểm tra thứ tự mũi tiêm cho PRIMARY và BOOSTER (nhưng không block nếu là ADDITIONAL)
                ...
                return Promise.reject(
                    new Error(
                        `Phải tiêm mũi ${
                            ...
                            value - 1
                        } ở nơi khác, vui lòng chọn loại liều "Tiêm bù" hoặc "Bổ sung" thay vì "${getDoseLabel(
                            doseType
                        )}".`
                    )
                );
        ...
        // Đưa ra cảnh báo nếu có khả năng vượt quá số mũi tối đa, nhưng không block
        // Backend sẽ xử lý validation cuối cùng
        const maxDoses = selectedCampaign?.vaccine?.maxDoseCount;
        if (maxDoses && value > maxDoses) {
            // Chỉ warning, không reject để cho phép nurse tự quyết định
            console.warn(
                `Warning: Mũi ${value} có thể vượt quá giới hạn ${maxDoses} mũi của vaccine`
            );
        }
    ...
    const validateAdministeredDate = (_, value) => {
        ...
        if (selectedDate > todayEnd) {
            return Promise.reject(
                new Error("Không thể chọn ngày trong tương lai")
            );
        }
    ...
    const validateDoseAmount = (_, value) => {
        ...
        if (value > 2) {
            return Promise.reject(
                new Error("Liều lượng không được vượt quá 2ml")
            );
        }
            ...
            if (errorData?.errorCode) {
                const errorMessages = {
                    ...
                    DUPLICATE_VACCINATION:
                        "Học sinh đã được tiêm trong chiến dịch này",
                ...
                errorMessage =
                    errorMessages[errorData.errorCode] ||
                    errorData.error ||
                    errorMessage;
                ...
                return {
                    ...
                    canAdminister:
                        ...
                        (dose.doseOrder > 1 &&
                            studentHistory.some(
                                (record) =>
                                    record.doseOrder === dose.doseOrder - 1
                            )),
                };
            }
        }
    ...
    const handlePerformVaccination = async (student) => {
        ...
        const suggestedDoseOrder =
            nextDose?.doseOrder ||
            (studentVaccinationHistory.length > 0
                ? Math.max(
                      ...studentVaccinationHistory.map((h) => h.doseOrder)
                  ) + 1
                : 1);
            ...
            <div style={{ marginBottom: 16 }}>
                ...
                >
                    ...
                    <div style={{ fontSize: 14, color: "#666" }}>
                        ...
                        {scheduleDisplay.filter((d) => d.isCompleted).length}{" "}
                        mũi | Còn lại:{" "}
                        {scheduleDisplay.filter((d) => !d.isCompleted).length}{" "}
                        mũi
                    </div>
                            ...
                            {dose.isCompleted ? (
                                <div style={{ fontSize: 12, color: "#52c41a" }}>
                                    ...
                                    {dose.administeredDate
                                        ? dayjs(dose.administeredDate).format(
                                              "DD/MM/YYYY"
                                          )
                                        : "N/A"}
                                </div>
                                ...
                                <div style={{ fontSize: 12, color: "#faad14" }}>
                                    ⏳ Chưa đủ thời gian (cần{" "}
                                    {dose.minInterval || 0} ngày)
                                </div>
                            ) : (
                    ...
                    <Alert
                        ...
                        description={
                            canReceive
                                ...
                                : `Cần đợi thêm ${
                                      nextDose.minInterval || 0
                                  } ngày sau mũi ${nextDose.doseOrder - 1}`
                        }
        ...
        <div className="space-y-6">
                    ...
                    if (!nextDose && studentVaccinationHistory.length > 0) {
                        ...
                        return (
                            <Alert
                                ...
                                description="Học sinh đã tiêm đủ theo phác đồ mũi tiêm. Nếu cần tiêm mũi bổ sung hoặc nhắc lại, vui lòng nhập số mũi tiêm thủ công và chọn loại liều phù hợp (Bổ sung/Nhắc lại)."
                                type="warning"
                                showIcon
                                style={{ marginBottom: 16 }}
                            />
                    ...
                    >
                        <DatePicker
                            ...
                            disabledDate={(current) => {
                                ...
                                return (
                                    current &&
                                    (current.toDate() < todayStart ||
                                        current.toDate() > todayEnd)
                                );
                            }}
                    ...
                    >
                        <InputNumber
                            ...
                            onChange={() => {
                                ...
                                setTimeout(() => {
                                    vaccinationForm.validateFields([
                                        "doseOrder",
                                    ]);
                                }, 100);
                            }}
            ...
            >
                ...
                >
                    ...
                    <Form.Item
                        name="doseType"
                        label="Loại liều (không thể thay đổi)"
                    >
                    ...
                    <Form.Item name="followUpRequired" label="Cần theo dõi">
                        <Select
                            placeholder="Chọn tình trạng theo dõi"
                            allowClear
                        >
