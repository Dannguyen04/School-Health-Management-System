...
const StudentTreatment = () => {
    ...
    const scheduleColumns = [
        ...
        },
        {
            ...
            render: (_, record) => {
                if (
                    !record.todaySchedules ||
                    record.todaySchedules.length === 0
                )
        ...
        return scheduledTreatments.filter((item) => {
            ...
            if (filterStatus !== "all") {
                if (!item.todaySchedules || item.todaySchedules.length === 0) {
                    ...
                    else if (filterStatus === "not_given")
                        statusMatch = anyFuture;
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (
                timesGiven === maxTimes &&
                totalGiven + dosageToGive <= dailyLimit
            ) {
                setSubmitting(false);
                Modal.confirm({
                    title: "Cảnh báo",
            ...
            <div className="flex justify-between items-center">
                <div>
                    <Title level={2} className="mb-2">
                        Quản lý lịch phát thuốc từ phụ huynh
                    </Title>
                    <Text type="secondary">
                            ...
                            },
                            {
                                validator: (_, value) => {
                                    if (
                                        ...
                                        selectedTreatment &&
                                        value >
                                            parseFloat(selectedTreatment.dosage)
                                    ) {
                                    ...
                                    if (
                                        ...
                                        typeof selectedTreatment.timesGivenToday ===
                                            "number"
                                    ) {
                                        const maxTimes =
                                            selectedTreatment.customTimes
                                                .length;
                                        const timesGiven =
                                            selectedTreatment.timesGivenToday;
