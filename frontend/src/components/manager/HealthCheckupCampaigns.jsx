...
const HealthCheckupCampaigns = () => {
    ...
    const fetchCampaigns = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                message.error(
                    res.data.error || "Không thể tải danh sách chiến dịch"
                );
            }
        } catch {
    ...
    const updateCampaign = async (id, data) => {
        try {
            ...
            if (res.data.success) {
                ...
                message.error(
                    res.data.error || "Không thể cập nhật chiến dịch"
                );
                return false;
            }
        } catch (err) {
    ...
    const deleteCampaign = async (id) => {
        try {
            ...
            message.error(
                err.response?.data?.error || "Không thể xóa chiến dịch"
            );
            return false;
        }
    };
    ...
    const updateCampaignStatus = async (id, status) => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                message.error(
                    res.data.error || "Không thể cập nhật trạng thái"
                );
            }
        } catch (err) {
            message.error(
    ...
    const doNotifyParents = async (id) => {
        ...
        try {
            ...
            if (res.data.success) {
                message.success(
                    res.data.message || "Đã gửi thông báo thành công!"
                );
            ...
            message.error(
                err.response?.data?.error || "Không thể gửi thông báo"
            );
            setNotifyModal((prev) => ({ ...prev, loading: false }));
        }
    };
                    ...
                    <div>
                        <ul>
                            ...
                            <li>
                                <b>Thời gian:</b>{" "}
                                {dayjs(
                                    notifyModal.campaign.scheduledDate
                                ).format("DD/MM/YYYY")}{" "}
                                -{" "}
                                {dayjs(notifyModal.campaign.deadline).format(
                                    "DD/MM/YYYY"
                                )}
                            </li>
                        </ul>
                        <p style={{ marginTop: 8 }}>
                            <b>Lưu ý:</b> Thông báo sẽ được gửi tới tất cả phụ
                            huynh có con thuộc các khối này.
                        </p>
                    </div>
                )}
            </AntdModal>
