    ...
    const createCampaign = async (data) => {
        try {
            ...
            if (response.data.success) {
                ...
                message.error(
                    response.data.error || "Không thể thêm chiến dịch"
                );
    ...
    const updateCampaign = async (id, data) => {
        try {
            ...
            if (response.data.success) {
                ...
                message.error(
                    response.data.error || "Không thể cập nhật chiến dịch"
                );
                return false;
            }
        } catch (error) {
    ...
    const deleteCampaign = async (id) => {
        try {
            ...
            if (response.data.success) {
                ...
                message.error(
                    response.data.error || "Không thể xóa chiến dịch"
                );
                return false;
            }
        } catch (error) {
            ...
            message.error(
                error.response?.data?.error || "Không thể xóa chiến dịch"
            );
            return false;
        }
    };
    ...
    const handleSendConsentClick = async (campaign) => {
        ...
        try {
            const res = await axios.get(
                "/api/manager/students/grades-with-count",
                {
                    headers: getHeaders(),
                }
            );
    ...
    const handleViewConsents = async (campaign) => {
        try {
            ...
            if (response.data.success) {
                ...
                message.error(
                    response.data.error ||
                        "Không thể tải danh sách phiếu đồng ý"
                );
            }
        } catch (error) {
            message.error(
            ...
            render: (_, record) => (
                <Space>
                    ...
                    <Popconfirm
                        ...
                        onConfirm={() =>
                            updateCampaignStatus(record.id, "FINISHED")
                        }
                        onCancel={() =>
                            updateCampaignStatus(record.id, "CANCELLED")
                        }
                    >
        ...
        if (values.name?.trim()) {
            filtered = filtered.filter((c) =>
                c.name
                    .trim()
                    .toLowerCase()
                    .includes(values.name.trim().toLowerCase())
            );
        }
    ...
    const updateCampaignStatus = async (id, status) => {
        try {
            ...
            if (response.data.success) {
                ...
                message.error(
                    response.data.error || "Không thể cập nhật trạng thái"
                );
            }
        } catch (error) {
            message.error(
                    ...
                    onSubmit={async (
                        ...
                        if (selectedCampaign) {
                            ...
                            if (!selectedVaccination) {
                                message.error(
                                    "Vui lòng chọn loại vaccine hợp lệ"
                                );
                                setSubmitting(false);
                                return;
                            }
                            ...
                            <Form layout="vertical" onFinish={handleSubmit}>
                                    ...
                                    >
                                        <Descriptions column={1} size="small">
                                            ...
                                            <Descriptions.Item label="Nguồn gốc">
                                                {selectedVaccination.origin ||
                                                    "Không có"}
                                            </Descriptions.Item>
                                            <Descriptions.Item label="Yêu cầu">
                                                {selectedVaccination.requirement ===
                                                "REQUIRED"
                                                    ? "Bắt buộc"
                                                    : "Tùy chọn"}
                                            </Descriptions.Item>
                                            ...
                                            <Descriptions.Item label="Chống chỉ định">
                                                {selectedVaccination.contraindications ||
                                                    "Không có"}
                                            </Descriptions.Item>
                                            <Descriptions.Item label="Độ tuổi khuyến nghị">
                                                {selectedVaccination.recommendedAge ||
                                                    "Không có"}
                                            </Descriptions.Item>
                                            ...
                                            <Descriptions.Item label="Số liều tối đa">
                                                {selectedVaccination.maxDoseCount ||
                                                    "Không có"}
                                            </Descriptions.Item>
                                        </Descriptions>
                                    </Card>
                                ...
                                >
                                    <Select
                                        ...
                                        onChange={(val) =>
                                            setFieldValue("targetGrades", val)
                                        }
                                        onBlur={handleBlur}
            ...
            >
                <div>
                    ...
                    <p style={{ marginTop: 16 }}>
                        Bạn có chắc chắn muốn gửi phiếu cho các khối đã chọn
                        không?
                    </p>
                </div>
            </Modal>
