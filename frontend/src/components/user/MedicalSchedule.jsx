...
const MedicalSchedule = () => {
  ...
  const fetchVaccinationDetail = async (campaignId, studentId) => {
    try {
      setLoading(true);
      const res = await parentAPI.getVaccinationDetail(campaignId, studentId);
      if (res.data.success) {
      ...
      <div className="w-full max-w-5xl mx-auto px-4 pt-24">
        <div className="text-center mb-4">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Xem lịch tiêm chủng và lịch khám sức khỏe của học sinh để theo dõi
            quá trình bảo vệ sức khỏe.
          </p>
        </div>
        <Tabs
          activeKey={tab}
